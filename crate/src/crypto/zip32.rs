...
mod tests {
    ...
    fn can_derive_shielded_key_to_serialized() {
        ...
        let payment_address: PaymentAddress = BorshDeserialize::try_from_slice(payment_address)
            .expect("Should be able to deserialize payment address!");
        let xsk: ExtendedSpendingKey = BorshDeserialize::try_from_slice(xsk)
            .expect("Should be able to deserialize extended spending key!");
        let xfvk: ExtendedFullViewingKey = BorshDeserialize::try_from_slice(xfvk)
